...
class XmlImport
{
		...
		if ($sLocalImages != $sRemoteImages) {
			...
			foreach ($aImages as $k=>$v) {
				...
				if ($imageLocation == 'remote') {
					...
					$origen = $v['url'];
					$destination = '../images/viviendas/' . $id . '/';
					$this->downloadImage($origen, $destination, $fileName);
	...
	public function downloadImage($origen, $destination, $fileName)
	{
		...
		$this->pictureresize($origen, "$destination/s_$fileName", $small_image_h, $small_image_v);
		$this->pictureresize($origen, "$destination/m_$fileName", $medium_image_h, $medium_image_v);
		$this->pictureresize($origen, "$destination/l_$fileName", $large_image_h, $large_image_v);
		$this->pictureresize($origen, "$destination/g_$fileName", $giant_image_h, $giant_image_v);
